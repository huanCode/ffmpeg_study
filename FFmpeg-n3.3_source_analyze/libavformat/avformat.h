...
typedef struct AVIndexEntry {
    int64_t pos;		//stco current chunk offset
    int64_t timestamp;  //stts 的累加前面所有的delta
          /**<
...
#define AVINDEX_DISCARD_FRAME  0x0002    /**
                                          ...
                                          */
    int flags:2;	//AVINDEX_KEYFRAME 关键字; 0 不是关键帧

    //stsz 对应某一个sample size
    int size:30; //Yeah, trying to keep the size of this small to reduce memory requirements (it is 24 vs. 32 bytes due to possible 8-byte alignment).
    int min_distance;         //距离前面的关键帧的帧数
...
#define MAX_REORDER_DELAY 16
    ...
    AVIndexEntry *index_entries; /**   stts < Only used if the format does not support seeking natively.  */
    int nb_index_entries;		//stts 
