...
typedef struct MOVStreamContext {
    ...
    int *keyframes;     //stss的entries
    int time_scale;
    int64_t time_offset;  ///< time offset of the edit list entries
    int current_sample;     //stsz当前的第几个sample
    ...
    int64_t track_end;    ///< used for dts generation in fragmented movie files ，stts 所有sample-delta累加
    int start_pad;        ///< amount of samples to skip due to enc-dec delay
    unsigned int rap_group_count;
    MOVSbgp *rap_group;

    int nb_frames_for_fps;  //stts 所有sample-delta累加
    int64_t duration_for_fps;   //stts 所有sample-count累加
...
typedef struct MOVContext {
    ...
    int trak_index;       ///< Index of the current 'trak' ,sdh: == AVStream::index == MOVStreamContext::ffindex
    char **meta_keys;
